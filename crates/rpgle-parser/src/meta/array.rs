// vendored/inspired by: https://github.com/near/nearcore/blob/daf321caa479d7b2a0f3fcad1304945f399fcfca/utils/stdx/src/lib.rs
...
//     assert_eq!(
//         (&mut [0, 1], &mut [2, 3, 4][..]),
//         split_slice_mut(&mut [0, 1, 2, 3, 4])
//     );
//     assert_eq!(
//         (&mut [0, 1][..], &mut [2, 3, 4]),
//         rsplit_slice_mut(&mut [0, 1, 2, 3, 4])
//     );
// }
//
...
//     assert_eq!(
//         (&[[0, 1], [2, 3]][..], &[4][..]),
//         as_chunks::<2, _>(&[0, 1, 2, 3, 4])
//     );
//     assert_eq!(
//         Ok(&[[0, 1], [2, 3]][..]),
//         as_chunks_exact::<2, _>(&[0, 1, 2, 3])
//     );
//     assert_eq!(
//         Err(InexactChunkingError {
//             slice_len: 5,
...
// zzz: customize
/// Asserts, at compile time, that `S == A + B + C`.
struct AssertEqSum3<const S: usize, const A: usize, const B: usize, const C: usize>;
impl<const S: usize, const A: usize, const B: usize, const C: usize> AssertEqSum3<S, A, B, C> {
    const OK: () = assert!(S == A + B + C);
}

/// Splits `&[char; L + X + R]` into `(&[char; L], &[char; X], &[char; R])`.
pub fn split_array3<const N: usize, const L: usize, const X: usize, const R: usize>(
    xs: &[char; N],
) -> (&[char; L], &[char; X], &[char; R]) {
    #[allow(clippy::let_unit_value)]
    let () = AssertEqSum3::<N, L, X, R>::OK;

    let (left, rest) = xs.split_at(L);
    let (x, right) = rest.split_at(X);
    (
...
pub fn pluck_array3<const N: usize, const L: usize, const X: usize, const R: usize>(
    xs: &[char; N],
) -> &[char; X] {
    let (_, x, _): (&[char; L], &[char; X], &[char; R]) = split_array3(xs);
    x
}
